...
class Estimate_params(Prepare_data):
    ...
    def run_epsilon(self, modeltype:'str'):
        ...
        df_OLS = df_OLS.query('Pi_ij!=0 & W_ij!=0')
        # df_OLS = df_OLS.query('LX_i!=LE_j')
        # df_OLS = df_OLS[(df_OLS['t_ij'] < 60/(60*24))]
        df_OLS['LPi_ij'] = df_OLS['Pi_ij'].apply(np.log)
        df_OLS['LW_ij'] = df_OLS['W_ij'].apply(np.log)
        display(df_OLS)
        self.df_OLS = df_OLS
        df_OLS = df_OLS.set_index(['LX_i','LE_j'])
        # x, y, 重みwの算出
        # w = df_Pi[(df_Pi['value']!=0) & (df_W['value']!=0)]['value']
        ...
        # input fixed effect
        self.res = model_sm
        fix_eff = model_sm.estimated_effects.reset_index()
        df_XE = pd.DataFrame(0, index=list(range(0,count)), columns=list(range(0,count)))
        for i in fix_eff.itertuples(): df_XE.at[i[1], i[2]] = np.exp(i[3])
        K = df_XE.at[0,0]
        df_XE = df_XE / K 
        ...
        self.ref_next['dPi_ij'] = (Pi_ij_est-df_OLS['Pi_ij'].to_numpy()) / df_OLS['Pi_ij'].to_numpy()*100
        # 推定結果の確認, 傾きが𝛾𝜖を表す
        ...
        print('ΔPi_est_ij (%)')
        display(pd.DataFrame(self.ref_next['dPi_ij']))
        print('ΣPi_ij')
    ...
    def moment_eta_rho(self, eta, rho): # moment function
        ...
        beta_cns_ij_prev = self.exog_prev['beta_cns_ij']
        beta_flr_ij_prev = self.exog_prev['beta_flr_ij']
        beta_chd_ij_prev = self.exog_prev['beta_chd_ij']
        t_ij_prev = self.exog_prev['t_ij']
        K_i_prev = self.exog_prev['K_i']
        # set variables in the next priod
        ...
        beta_cns_ij_next = self.exog_next['beta_cns_ij']
        beta_flr_ij_next = self.exog_next['beta_flr_ij']
        beta_chd_ij_next = self.exog_next['beta_chd_ij']
        t_ij_next = self.exog_next['t_ij']
        K_i_next = self.exog_next['K_i']
        # calulate the structural residuals in the previous priod
        EN_R_prev = N_R_i_prev / gmean(N_R_i_prev)
        W_prev = np.sum((v_ij_prev/ (p_i_prev.reshape(1, -1).T**beta_cns_ij_prev * q_i_prev.reshape(1, -1).T**beta_flr_ij_prev * mu_ij_prev**beta_chd_ij_prev))**gameps, axis=1)
        ...
        W_next = np.sum((v_ij_next/ (p_i_next.reshape(1, -1).T**beta_cns_ij_next * q_i_next.reshape(1, -1).T**beta_flr_ij_next * mu_ij_next**beta_chd_ij_next))**gameps, axis=1)
    ...
    def recover_fundamentals(self):        
        ...
        Pi_est_ij = XE_ij * W_ij**gameps * B_i.reshape(1, -1).T**eps / np.sum(XE_ij * W_ij**gameps * B_i.reshape(1, -1).T**eps)
