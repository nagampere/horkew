...
class Solve_equilibrium(Estimate_params):
    ...
    def simultaneous_equations(self, l): # 未知数 w_jを解く方程式の定義
        ...
        phi_i = exog['phi_i']
        N_W_j_0 = exog['N_W_j_0']
        N_R_i_0 = exog['N_R_i_0']
        ...
        # if np.nan in q_i : q_i = self.ref_next['q_i'].tolist()
        # if np.nan in N_W_j : N_W_j = (self.ref_next['N_W_j']/N).tolist()
        # if np.nan in N_R_i : N_R_i = (self.ref_next['N_R_i']/N).tolist()
        ...
        N_R_i = N/np.sum(N_R_i) * N_R_i
        N_W_j = N/np.sum(N_W_j) * N_W_j
        ...
        # 業務用地の価格Q_jの定義
        Q_j = q_i * xi_i
        w_j = (alp*A_j**(1/alp))*((1-alp)/Q_j)**((1-alp)/alp)
    ...
    def solve_equilibrium(self, given_exog:dict, given_param:dict, modeltype:str, method:str, maxiter:int): # 外生変数から一般均衡を解く関数の定義
        ...
        N_W_j_init = (self.ref_next['N_W_j']/N).tolist()
        self.given_exog['N_W_j_0'] = N_W_j_init[0]
        N_W_j_init = N_W_j_init[1:n]

        N_R_i_init = (self.ref_next['N_R_i']/N).tolist()
        self.given_exog['N_R_i_0'] = N_R_i_init[0]
        N_R_i_init = N_R_i_init[1:n]
        x0_init = q_i_init + N_W_j_init + N_R_i_init
        ...
        b_N_w = [(0.000001, 0.1) for x in range(self.count-1)]
        b_N_r = [(0.000001, 0.1) for x in range(self.count-1)]
        bounds = tuple(b_q_i+b_N_w+b_N_r)
        ...
        q_i = np.array(result.x[0:n])
        N_W_j = np.append(self.given_exog['N_W_j_0'], result.x[n:(n*2-1)])
        N_R_i = np.append(self.given_exog['N_R_i_0'], result.x[(n*2-1):(n*3-2)])
