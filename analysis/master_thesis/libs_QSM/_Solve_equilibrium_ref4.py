...
class Solve_equilibrium(Estimate_params):
    ...
    def simultaneous_equations(self, l): # 未知数 w_jを解く方程式の定義
        ...
        for idx, x in enumerate(B_i): 
            if x < 0: B_i[idx] = abs(x)
        ...
        # 床面積の需給均衡から業務用地H_W_jの定義 (式(18)を参照)
        ...
        # 居住地アメニティの定義
        eq3 = B_i - bl_i*Omega_i**eta
        # 人口の均衡式(n本)
        # eq3 = N_R_i - N*np.sum(Pi_ij, axis=1)
        # 標準化
        # eq1 = eq1 / np.mean(supply_H_i)
        # eq2 = eq2 / np.mean(Q_j)
        # eq3 = eq3 / np.mean(N*np.sum(Pi_ij, axis=1))
    ...
    def solve_equilibrium(self, given_exog:dict, given_param:dict, modeltype:str, method:str, maxiter:int): # 外生変数から一般均衡を解く関数の定義
        ...
        B_i_init = self.ref_next['B_i'].tolist()
        N_R_i_init = self.ref_next['N_R_i'].tolist()
        x0_init = w_j_init + q_i_init + B_i_init
        ...
        b_q_i = [(0.1,20) for x in range(self.count)]
        b_B_i = [(0.45,0.62) for x in range(self.count)]
        # b_N_r = [(0,N/10) for x in range(self.count)]
        bounds = tuple(b_w_j+b_q_i+b_B_i)
        ...
        if modeltype == 'minimize':
            if method=='L-BFGS-B': 
                ...
                result = opt.minimize(fun=self.objective_equations, x0=x0_init, tol=1e-9, method=method, options=options)
            ...
            result = opt.least_squares(fun=self.simultaneous_equations, x0=x0_init, ftol=1e-9, gtol=1e-14, method=method)
        else:
        ...
        # H_R_ijは0.1以上
        H_R_ij = beta_flr_ij * gam*L*v_ij / q_i.reshape(1,-1).T + mu_room*n_ij
        v_ij = np.where(H_R_ij>0.1, v_ij, np.zeros([self.count, self.count]))
        W_ij = v_ij/ (p_i.reshape(1, -1).T**beta_cns_ij * q_i.reshape(1, -1).T**beta_flr_ij * mu_ij**beta_chd_ij)
        Pi_ij = XE_ij * W_ij**gameps * B_i.reshape(1, -1).T**eps / np.sum(XE_ij * W_ij**gameps * B_i.reshape(1, -1).T**eps)
        # 労働量x_ijの定義 (式(8)を参照)
        # 労働需要M_W_jの定義 (式(15)を参照)
        N_R_i = N*np.sum(Pi_ij, axis=1)
        M_R_i =  N*np.sum(Pi_ij*x_ij, axis=1)
        M_W_j = N*np.sum(Pi_ij*x_ij, axis=0)
        # 一世帯当たり居住地面積H_R_ij, 居住地面積H_R_iの定義(式(9)を参照)
        H_R_i = np.sum(H_R_ij*N*Pi_ij, axis=1)
        ...
        eq['w_j'] = w_j
        eq['N_R_i'] = N_R_i
        eq['N_R_i_est'] = N*np.sum(Pi_ij, axis=1)
        eq['N_W_j'] = N_W_j
        eq['Ups_j'] = Ups_j
        eq['Omega_i'] = Omega_i
        eq['A_j'] = A_j
