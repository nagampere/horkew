...
class Horcher_Fertility_model():
    ...
    def __init__(self, count_i:int, count_j:int) -> None:
        ...
        # 外生的なパラメーター'beta_ij','delta_ij','theta_ij',
        self.param = {}
        self.param_keys = ['alpha','gamma','psi','mu_cost','mu_time','mu_room','L','T','N']
        ...
        self.ref_key_ij = ['lambda_ij','n_ij']
    ...
    def set_ref(self, ref:dict[str, np.ndarray]) -> None:
        ...
        # 外生的なパラメータδ_ijの算出
        self.exog['delta_ij'] = self.ref['n_ij']/(self.param['gamma']*self.param['L']*self.ref['v_ij']/self.ref['mu_ij'])
        self.exog['beta_ij'] = 4/5*(1-self.exog['delta_ij'])
        self.exog['theta_ij'] = 1/5*(1-self.exog['delta_ij'])
        ...
        print(self.exog['delta_ij'])
        print('beta_ij: 基本財の消費性向')
        print(self.exog['beta_ij'])
        print('theta_ij: 土地の消費性向')
        print(self.exog['theta_ij'])
    ...
    def estimate_epsilon(self, modeltype:'str'):
        ...
        x = np.log(self.ref['v_ij']/(self.exog['p_i'].reshape(1, -1).T**self.exog['beta_ij']*self.ref['q_i'].reshape(1, -1).T**self.exog['theta_ij']*self.ref['mu_ij']**self.exog['delta_ij']))
    ...
    def probabilities(self, l):
        ...
        utility = (self.ref['v_ij']/(self.exog['p_i'].reshape(1, -1).T**self.exog['beta_ij']*self.ref['q_i'].reshape(1, -1).T**self.exog['theta_ij']*self.ref['mu_ij']**self.exog['delta_ij']))**self.param['gam*eps']
    ...
    def solve_equilibrium(self, given_exog:dict, modeltype:str, method:str):
        ...
        eq['mu_ij'] = self.param['mu_cost']+eq['v_ij']*self.param['mu_time']+eq['q_i'].reshape(1,-1).T*self.param['mu_room']
        utility = (eq['v_ij']/(self.exog['p_i'].reshape(1, -1).T**self.exog['beta_ij']*eq['q_i'].reshape(1, -1).T**self.exog['theta_ij']*eq['mu_ij']**self.exog['delta_ij']))**self.param['gam*eps']
        ...
        # 一世帯あたり子供の数n_ijの算出
        eq['n_ij'] = self.exog['delta_ij'] * self.param['gamma']*self.param['L']*eq['v_ij']/eq['mu_ij']
        # 地域の子供の数n_iの算出
        eq['n_i'] = self.param['N']*np.sum(eq['lambda_ij']*eq['n_ij'], axis=1)
        # 居住人口N_R_iと就業人口N_W_jの算出 (式(14)を参照)
        ...
        # 一世帯当たり居住地面積H_R_ij, 居住地面積H_R_iの算出 (式(9)を参照)
        eq['H_R_ij'] = self.exog['theta_ij']*self.param['gamma']*self.param['L']*eq['v_ij']/eq['q_i'].reshape(1,-1).T + self.param['mu_room']*eq['n_ij']
