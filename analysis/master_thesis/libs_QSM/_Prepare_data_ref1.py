...
class Prepare_data():
    ...
    def equation_wage(self, l):
        ...
        eq = w_j - alp/(1-alp) * H_W_j/M_W_j * Q_j
        eq = np.sum(eq**2)
    ...
    def calibrate_wage(self, exog, ref):
        ...
        w_j_init = [1.0 for x in range(self.count)]
        x0_init = w_j_init
        ...
        b_w_j = [(0.5,10) for x in range(self.count)]
        # b_H_i = [(0.1,1) for x in range(self.count)]
        bounds = tuple(b_w_j)
        options = {'maxiter':10**5, 'maxfun':10**4, 'maxls': 2000, 'ftol': 1e-14, 'gtol': 1e-14}
        ...
        # H_W_j = H_R_i * the_i/(1-the_i)
        # w_j = alp/(1-alp) * H_W_j/M_W_j * Q_j
        w_j = np.array(result.x[0:self.count])
        print(result)
        return w_j
    ...
    def calc_dep(self, exog, ref):
        ...
        ref_dep['H_W_j'] = ref_dep['H_R_i'] / (1-ref['theta_i']) * ref['theta_i']
        exog_dep['phi_i'] = (ref_dep['H_R_i']/(1 - ref['theta_i'])) / exog['K_i']**(1-param['psi'])
        # 居住地の商業地利用の相対地価xi_i
